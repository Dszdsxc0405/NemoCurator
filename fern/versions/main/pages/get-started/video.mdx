...
* **FFmpeg 8.0+** with one of the following encoders:
  ...
  - CPU encoders: `libvpx-vp9`, or user-provided `libopenh264`/`libx264`
...
Use the maintained script in the repository to build and install `FFmpeg` with NVIDIA NVENC and `libvpx-vp9` support. The script enables dynamic CUDA/NVIDIA codec support through `--enable-cuda`, `--enable-cuvid`, `--enable-nvdec`, `--enable-nvenc`, and `--enable-ffnvcodec`, plus `--enable-libvpx`.
...
Confirm that `FFmpeg` is on your `PATH` and that the expected video encoders are available:

```bash
ffmpeg -hide_banner -version | head -n 5
ffmpeg -encoders | grep -E "h264_nvenc|libvpx-vp9|libopenh264|libx264" | cat
```
...
<Note>
On GPUs without NVENC hardware, such as A100 and H100, use `--transcode-encoder libvpx-vp9` or provide an FFmpeg build with `libopenh264` or `libx264` for software H.264 encoding.
</Note>
...
| **Encoding** |
| `--transcode-encoder` | `h264_nvenc`, `libvpx-vp9`, `libopenh264`, `libx264` | Video encoder for output clips. Use a CPU encoder on GPUs without NVENC such as A100/H100. `libopenh264` and `libx264` require a user-installed FFmpeg build — see [BYO H.264](../admin/installation.md#bring-your-own-h264-software-encoder-advanced). |
